...
import static com.pinyougou.casdemo.common.RedisKey.ACCESSTOKEN;
import static com.pinyougou.casdemo.common.RedisKey.AUTHORIZE;
import static com.pinyougou.casdemo.common.RedisKey.AUTHUSER;

/**
 * 第三步：服务端验证参数是否正确，生成access_token
 *
 * @author: wangsaichao
 * @date: 2018/5/27
 * @description: 访问令牌控制器
 * <p>
 * 代码描述：
 * 1、首先通过如http://localhost:9090/accessToken，
 * POST提交如下数据：
 ...
 * redirect_uri=http://localhost:9080/oauth-client/oauth2-login访问。
...
public class AccessTokenController {
    ...
    public HttpEntity token(HttpServletRequest request) throws OAuthSystemException {
        try {
            ...
            // 检查客户端安全Key是否正确
            Client client2 = clientService.findByClientSecret(oAuthTokenRequest.getClientSecret());
            if (client2 == null) {
            ...
            Object obj = shiroRedisUtils.get(AUTHORIZE + clientId);
            String redisauthCode = obj != null ? obj.toString() : "";
            ...
            if (oAuthTokenRequest.getParam(OAuth.OAUTH_GRANT_TYPE).equals(GrantType.AUTHORIZATION_CODE.toString())) {
                if (!redisauthCode.replace("\"", "").equals(authCode.replace("\"", ""))) {
                    OAuthResponse response = OAuthASResponse.errorResponse(HttpServletResponse.SC_BAD_REQUEST)
            ...
            shiroRedisUtils.set(ACCESSTOKEN + accessToken, accessToken);
            Object userobj = shiroRedisUtils.get(AUTHUSER + clientId);
            shiroRedisUtils.del(AUTHUSER + clientId);
            shiroRedisUtils.set(AUTHUSER + accessToken, userobj);
//            authorizeService.addAccessToken(accessToken, authorizeService.getUsernameByAuthCode(authCode));

            // 生成OAuth响应
            OAuthResponse response = OAuthASResponse.tokenResponse(HttpServletResponse.SC_OK)
                    .setAccessToken(accessToken).setExpiresIn(accessTokenExpiretime)
                    .buildJSONMessage();
